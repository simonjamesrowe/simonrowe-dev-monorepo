...
- [~] T086 Walk every step of [quickstart.md](./quickstart.md) against a local stack with restored production data, including the seven browser checks and the "blog narration is unchanged" section
  ...
  - **Not done**: the seven browser checks and the authenticated generation/audio steps. Both need an Auth0 login in a real browser session and a restored production dataset. Left for Simon.
