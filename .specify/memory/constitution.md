...
- AI-powered chat MUST use Spring AI 1.1.2 with Google Gemini as the
  LLM provider (`spring-ai-starter-model-google-genai`). The API key
  MUST be configured via the `GOOGLE_GENAI_API_KEY` environment
  variable. No other LLM provider SDK MAY be introduced.
...
| AI/LLM       | Spring AI + Google Gemini         | 1.1.2, gemini-3.1-pro-preview    |
