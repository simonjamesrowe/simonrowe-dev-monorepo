<!--
  ...
  Modified principles:
    - Principle II: Modern Java & React Stack
      Changed: AI provider from Google Gemini to OpenAI. Chat uses
      GPT 5.4 Nano via spring-ai-starter-model-openai. Embeddings
      use text-embedding-3-small for RAG vector search. API key
      env var changed from GOOGLE_GENAI_API_KEY to OPENAI_API_KEY.
      Added: Spring AI Elasticsearch vector store for RAG-based
      semantic search in chat (spring-ai-starter-vector-store-elasticsearch).
      Added: Docker redeploy capability — backend can restart
      Docker containers via admin UI using ProcessBuilder and
      Docker Compose CLI (mounted from host).
    - Principle VII: Backup & Restore
      Added: Google Drive remote backup/restore via Google API
      client. Backups uploaded as ZIP to configurable Drive folder.
      Added: Elasticsearch snapshot/restore using filesystem
      repository for vector embedding data.

  Added sections:
    - Technology Stack Constraints: new rows for Vector Store,
...
- AI-powered chat MUST use Spring AI 1.1.2 with OpenAI as the LLM
...
- Semantic search for chat MUST use Spring AI's Elasticsearch vector
  ...
  alongside keyword search indices. The `RetrievalAugmentationAdvisor`
...
| AI/LLM       | Spring AI + OpenAI                | 1.1.2, GPT 5.4 Nano (chat)      |
...
**Version**: 1.9.0 | **Ratified**: 2026-02-21 | **Last Amended**: 2026-04-05
