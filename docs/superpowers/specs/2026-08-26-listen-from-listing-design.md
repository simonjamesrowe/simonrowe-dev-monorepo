...
2. **`PublicLayout` wraps each route individually**, so any provider placed inside it
   ...
   must be a `new Audio()` held in a ref rather than a JSX `<audio>` — otherwise
   playback dies on the first route change, which is the specific thing a shared player is
   for.

   **Corrected during implementation:** this section originally said *detached*. A truly
   detached element is invisible to `document.querySelectorAll('audio')`, which only walks
   the document — see the correction under "Frontend: the bar". The element is created with
   `new Audio()` and appended to `<body>`: outside everything React reconciles, so it
   survives navigation, but still findable by the other player.
...
- **`NarrationAudioProvider`** — mounted above `<Routes>`, inside `AuthProvider` (it needs
  `getAccessToken`). Holds a single `HTMLAudioElement` appended to `<body>`, the current track,
...
Dismissing the bar during generation clears the bar. It does not cancel server-side work, and
it does not auto-play when the work lands — auto-play only follows a chain the reader is still
watching.

**Clarified during implementation.** This originally read "stops watching and clears the bar",
which is not implementable alongside the guarantee in "Who owns *this is now ready*" that the
card still flips without a page reload. The client poll *is* what learns the audio is ready, so
aborting it on dismiss left the card reading "Listen" until the next full page load even though
the MP3 existed — the opposite of what dismissing is meant to be safe about. The poll therefore
runs on to completion and records the result; "stops watching" means it stops being shown and
stops being auto-played, not that the loop stops. A dismissed track also suppresses the error
and the "taking a while" prompt, since there is no bar left to show them on.
...
handles that, and the provider installs the mirror image of it, so starting either player
pauses the other.

**Corrected during implementation.** This paragraph originally claimed "the bar's detached
element is a real `<audio>` to `document.querySelectorAll`". That is false:
`document.querySelectorAll` only traverses the document, so a detached element is invisible
to it and `NarrationPanel` would never have paused the bar — the two players could talk over
each other, which is the exact failure this paragraph was asserting was already handled. The
element is therefore appended to `document.body`. That keeps both properties that matter: it
is outside anything React reconciles, so navigation cannot unmount it, and it is in the
document, so both players can find each other. It is removed on provider unmount.
`NarrationPanel` itself needed no change.
